...
The publisher emits `{{< notice >}}` (callouts) and `{{< mermaid >}}` shortcodes, but this site defines only `latin-motto.html` under `layouts/shortcodes/`. No existing post uses the missing shortcodes, so the gap is latent; any future note with a callout or mermaid block would publish broken. Fix direction is undecided: either add `notice.html` + `mermaid.html` here, or change the publisher to emit plain markdown and raw HTML.
